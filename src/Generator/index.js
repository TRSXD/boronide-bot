...
const encoder = require('./../Bytecode/encoder')
...
VM.__init__(0, '|Const|', '|Inst|', '|Proto|', ${tree.Chunk.Args})
        ...
        */
        ...
        serializeProto = (proto) => {
            let _s = ''
            _s += encoder.dump_char(proto.Upvals)
            _s += encoder.dump_char(proto.Args)

            _s += String.fromCharCode(types.constList)
            _s += encoder.from_int(proto.Const.length)
            for (let i in proto.Const) {
                let v = proto.Const[i]
                let me = ''
                console.log(v)
                switch (typeof v) {
                    ...
                    case 'undefined':
                        if (v === null || v === undefined) {
                            me += String.fromCharCode(types.nil)
                            break
                        }

                        if (v.constructor === Array) {
                            me += String.fromCharCode(types.string)
                            me += encoder.dump_string(fromCode2Str(v))
                            break
                        }
                    case 'object':
                        switch (v.Type) {
                            case 'string':
                                me += String.fromCharCode(types.encrypted)
                                me += encoder.dump_string(fromCode2Str(v.Data))
                                break
                            default: 
                                console.log(v)
                                throw `Unexpected type, ${v.Type}`
                        }
                       
                        break
                
                    case 'boolean':
                        me += v ? String.fromCharCode(types.bool_true) 
                            : String.fromCharCode(types.bool_false)
                        break

                    case 'number':
                        me += String.fromCharCode(types.number)
                        me += encoder.from_double(v)
                        console.log(encoder.from_double(v).length)
                        break

                    case 'string':
                        me += String.fromCharCode(types.string)
                        me += encoder.dump_string(v)
                        break
                }
                _s += me
            }

            _s += String.fromCharCode(types.instList)
            _s += encoder.from_int(proto.Instr.length)
            let instCache = {}
            let cIdx = 0
            let s = ''
            for (let i in proto.Instr) {
                let inst = proto.Instr[i]
                if (inst === null)
                    continue;
                //console.log("-1>", inst.Name, inst['1'], inst['2'], inst['3'])
                updateRegisters(inst, proto)
                //console.log("-2>", inst.Name, inst['1'], inst['2'], inst['3'])
                ...
                s += (String.fromCharCode(eTypes[inst.Type])); // Type
                if (inst.Type === 'SOP' || inst.Type === 'NOP') {
                    s += `\\${from_int(inst.Value, 5).join('\\')}`;
                    if (!inCache) {
                        instCache[inst.Enum] = cIdx;
                    ...
                    if (typeof inst['1'] === 'boolean')
                        s += `\\1${inst['1'] === true ? String.fromCharCode(BooleanEnc.True) : String.fromCharCode(BooleanEnc.False)}`;
                    else
                        s += `\\0\\${from_int(inst['1'] === null ? 0 : inst['1'], 1).join('\\')}`;
        
                    if (typeof inst['2'] === 'boolean')
                        s += `\\1${inst['2'] === true ? String.fromCharCode(BooleanEnc.True) : String.fromCharCode(BooleanEnc.False)}`;
                    else {
                        let n = inst['2'] === null ? 0 : inst['2'];
                        if (inst.Type === 'AsBx')
                            n = n + 131071;
                    ...
                    if (inst['3'] !== null) {
                        if (typeof inst['3'] === 'boolean')
                            s += `\\1${inst['3'] === true ? String.fromCharCode(BooleanEnc.True) : String.fromCharCode(BooleanEnc.False)}`;
                        else
                            s += `\\0\\${from_int(inst['3'] === null ? 0 : inst['3'], 3).join('\\')}`;
                    }
            ...
            _s += s
            _s += String.fromCharCode(types.end)
            return _s
        }
        ...
        let enc = (s) => {
            ...
            for (let i = 0; i < s.length; i++) {
                ...
                if (((code >= 32 && code <= 126) || (code >= 128 && code <= 254)) && !blockedChars.includes(code))
                    e += v
                else
                    e += `\\${parseInt(code, 16)}`
            }
        ...
        let genProtoDes = () => {
            ...
            d += `
            local function deserializeProto(str)
                local t = { }
                local p = 1
                local l = #str - 1
                ...
                local gBits16 = function()
                    local x, y = byte(str, p, p + 1)
                    p = p + 2
                    return (y * 256) + x
                end	
        
                local gBits24 = function()
                    local x, y, z = byte(str, p, p + 2)
                    p = p + 3
                    return (z * 65536) + (y * 256) + x
                end	
            
                local gBits32 = function()
                    local w, x, y, z = byte(str, p, p + 3)
                    p = p + 4
                    return (z * 16777216) + (y * 65536) + (x * 256) + w;
                end
        
                local gBits40 = function()
                    local w, x, y, z, a = byte(str, p, p + 4)
                    p = p + 5
                    return (z * 16777216) + (y * 65536) + (x * 256) + w
                        + (a * 4294967296);
                ...
                local pi, ii, ci = 0, 0, 0
                local proto = {['|Const|'] = {}, ['|Inst|'] = {}, ['|Proto|'] = {}}
                proto['|T_UPVALS|'] = read():byte()
                proto['|Args|'] = read():byte()

                local usedInstsCache = {}
                while (true) do
                    local c = read():byte()

                    if (c == ${types.constList}) then
                        local len = gBits32()
                        for i = 0, len - 1 do
                            ci = ci + 1
                            local c = nil
                            ...
                            if t == ${types.nil} then
                                print("NIL", nil)
                            elseif t == ${types.bool_false} then
                                print("BOOL:", false)
                            elseif t == ${types.bool_true} then
                                print("BOOL:", true)
                            elseif t == ${types.number} then
                                print("NUM:", gFloat())
                            elseif t == ${types.string} then
                                print("STRING:", read(l))
                            elseif t == ${types.encrypted} then
                                print("EN_S:", read(l))
                            else
                                print("Tg:", t)
                            end
        ...
        protoLoadCode.push(genProtoDes())

        let protos = []
        for (let idx in tree.Chunk.Proto) {
            let proto = tree.Chunk.Proto[idx]
            let str = serializeProto(proto)
            protos.push(str)
            protoLoadCode.push(`local __A__ = deserializeProto("${toEscStrF(str)}")`)
        }
