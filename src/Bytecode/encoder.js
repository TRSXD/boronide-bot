...
const Float32ToHex = (float32) => {
    const getChar = i => String.fromCharCode(i)
    var view = new DataView(new ArrayBuffer(4))
    view.setFloat32(0, float32);
    return Array.apply(null, { length: 4 }).map((_, i) => getChar(view.getUint8(i))).join('');
}
...
let encoder = {
    ...
    dump_string: (s) => {
        if (typeof s == 'string')
            return `${encoder.from_int(s.length + 1)}${encoder.from_string(s)}\0`
        else
            return `${encoder.from_int(1)}\0`
    },
    ...
    dump_instruction: (Inst) => {
        ...
        let s = ''
        let sinst = ''
        if (Inst.Name == 'TEST' || Inst.Name == 'TESTSET') {
            Inst[3] = Inst[3] ? 0 : 1
        } else if(Inst.Name == 'EQ' || Inst.Name == 'LT' || Inst.Name == 'LE') {
            Inst[1] = Inst[1] ? 1 : 0
        }
